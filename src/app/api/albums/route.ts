...
export async function POST(request: NextRequest) {
  try {
    ...
    const musicUrlFromClient = formData.get('musicUrl') as string | null;
    ...
    // Music handling: prefer client-pre-uploaded URL to avoid large multipart payloads
    let musicUrl = '';
    if (typeof musicUrlFromClient === 'string' && musicUrlFromClient.trim().length > 0) {
      musicUrl = musicUrlFromClient.trim();
    } else if (music && music.size > 0) {
      try {
